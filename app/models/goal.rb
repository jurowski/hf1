...
class Goal < ActiveRecord::Base
  ...
  def is_active
    ...
    if self.status != "hold" and (((self.start <= self.user.dtomorrow) and (self.stop >= self.user.dtoday)) or ((self.laststatusdate != nil) and (self.laststatusdate > self.user.dstop_after_stale_days)))
        return true        
    else
        return false
    end
  ...
  def stats_image
      ...
      width = 100
        ...
        if self.has_unanswered_checkpoints
          ...
          image << '<img src="/images/question.png" alt="Missing checkpoints" width=25 border="0"/><br><font color=red><br>To get full stats, first update your missing checkpoints.</font></a></strong></br>'
        else
            if self.forged_in
              image << '<img src="/images/ring_buttons21.png" alt="Established Habit" width=' + width.to_s + 'px border=0/>'
            else
