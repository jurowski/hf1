...
states.each do |state|
  ...
  output_me("info","working on " + state.country + ":" + state.state_code )
    ...
    state_data += " <br><strong>#{number_with_delimiter(total_participating, :delimiter => ',')} Participants</strong>"
    state_data += " <br><strong>#{number_with_delimiter(challenge_lbs_lost_total, :delimiter => ',')} lbs.</strong> lost so far"
    state_data += " <br><strong>#{challenge_percent_of_goal_met}%</strong> of Goal Reached"
    state_data += " <br>Challenge Rank (US & Canada): <strong>##{challenge_number_rank} out of #{states.size}</strong>"
    state_data += " <br>"
    ...
    state_data += " <br>"
    state_data += " <br>"
    state_data += " <div style=\"background-color:#ffffff;\">"
