...
require 'date'
require 'logger'
class UpdatePromotionNewPaymentMonthly < ActiveRecord::Base
  # This script emails people who had been paying InfusionSoft at $3.95/month and who can now use PayWhirl @ $1.29/month

  ### RUN IN DEV:
  ### rvm use 1.8.7;cd /home/sgj700/rails_apps/hf1/;ruby script/runner app/update_promotion_new_payment_monthly.rb

  ### RUN IN PRODUCTION:
  ### cd /habitforge/current;RAILS_ENV=production /usr/bin/ruby /home/jurowsk1/etc/rails_apps/habitforge/current/script/runner /home/jurowsk1/etc/rails_apps/habitforge/current/app/update_promotion_new_payment_monthly.rb
  #RAILS_ENV=production 
  #/usr/bin/ruby 
  #/home/jurowsk1/etc/rails_apps/habitforge/current/script/runner 
  #/home/jurowsk1/etc/rails_apps/habitforge/current/app/update_promotion_new_payment_monthly.rb
  ...
  begin
    ...
    if testing == 1 ### assuming adminuseremail of "jurowski@gmail.com" or "jurowski@pediatrics.wisc.edu"
      ...
      user_conditions = "plan = 'Monthly @ $3.95'"
    end
    ...
    for user in @users
        if count_emailed < maxemails
            run_once = 0
        		if user.promotion_new_payment_monthly_sent == nil or testing == 1
                ...
                the_subject = "Reduce your HabitForge Monthly Payment by 67% Now!!!"
                Notifier.deliver_promotion_new_payment_monthly(user, the_subject) # sends the email  

                puts "#{user.email} was sent the promotion_new_payment_monthly email"

                user.promotion_new_payment_monthly_sent = dnow
                user.save 
