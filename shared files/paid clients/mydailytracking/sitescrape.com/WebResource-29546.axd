...
// These vars are defined in control and hence not needed here.
//var shim='/App_Themes/Theme_100002/Image/x.gif';
//var shim_pattern=/App_Themes/Theme_100002/Image/x\.gif$/i;
var root=false;
...
obj.style.filter="progid:DXImageTransform.Microsoft.AlphaImageLoader(src='"+src+"', sizingMethod='"+mode+"')";obj.style.backgroundImage='url('+shim+')';};var el_fnFixPng=function(img){var src=img.src;img.style.width=img.width+"px";img.style.height=img.height+"px";
if(img.width==0 && img.attributes["width"] !=null){img.style.width =  img.attributes["width"].value + "px";} if(img.height==0 && img.attributes["height"] !=null){img.style.height = img.attributes["height"].value + "px";} img.style.filter="progid:DXImageTransform.Microsoft.AlphaImageLoader(src='"+src+"', sizingMethod='scale')";img.src=shim;};
